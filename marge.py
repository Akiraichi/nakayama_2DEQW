...
#
#     # プロットする
#     ax = df.loc["t=" + str(step_t_list[0])].plot_x_axis_is_index(grid=True, x="t=0", figsize=(8, 6))  # step_tステップ目のデータを抽出してプロットする
#     ax.set_xlabel("$t_{erase}$", size=24, labelpad=5)
#     ax.set_ylabel(f"${label}$", size=24)
#
#     for i in range(1, len(step_t_list)):
#         df.loc["t=" + str(step_t_list[i])].plot_x_axis_is_index(grid=True, x="t", figsize=(8, 6), ax=ax)  # 同じ表に次々とプロットしていく
#     plt.legend()
#     # plt.show()
...
#     # index番号以降のデータを抽出する
#     df = df[index:]
#     ax = df.plot_x_axis_is_index(grid=True, x="t", figsize=(8, 6))
...
#         self.file_name = None
#         if type == "KL":
#             folder_list = glob.glob(f'result/KL_div/*')
#
#         elif type == "prob":
...
#
#     def __load(self):
#         path = glob.glob(f'{self.path}/*.jb')
#         self.data = joblib.load(path[0])
#
#     def plot(self):
#         print(self.data)
#         # 最もKLダイバージェンスが低い時間tの推移をプロットする。横軸が時間ステップt、縦軸がKLダイバージェンス
#
#
#
#
